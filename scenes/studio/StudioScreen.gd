...
func _ready() -> void:
    ...
    var studio_emblem := CompanyEmblem.new().configure(GameState.company_name, GameState.founder_name)
    studio_emblem.custom_minimum_size = Vector2(38, 38)
    company_label.get_parent().add_child(studio_emblem)
    company_label.get_parent().move_child(studio_emblem, company_label.get_index())
